...
public function getProductList_byMultiLevel_orderProductId_filter($idProductCat,$order_productId,$trang, $limit, $page){
    ...
    if ($idProductCat != "" && $nameColIdProductCat != "") {
        ...
        if ($product_weight != 0) {
            $where .= " AND product_weight = $product_weight";
            $cond .= "&weight=$product_weight";
        }

        if (isset($_GET['box'])) {
            $count_box = count($_GET['box']);
            if ($count_box != 3) {
                $where .= " AND (1=0";
                foreach ($_GET['box'] as $item) {
                    $where .= "  OR hop_so = $item";
                    $cond .= "&box[]=$item";
                }
                $where .= ")";
            }
        ...
        if (isset($_GET['price'])) {
            $count_price = count($_GET['price']);
            if ($count_price != 2) {
                $product_price = $_GET['price'][0];
                if ($product_price == 1) {
                    $where .= " AND product_price between 0 and 600000000";
                } else {
                    $where .= " AND product_price > 600000000";
                }
                $cond .= "&price[]=$product_price";
            }
        }
...
public function getProductList_byMultiLevel_orderProductId_Sale($idProductCat,$order_productId,$trang, $limit, $page){
    ...
    if ($idProductCat != "" && $nameColIdProductCat != "") {
        ...
        $cond = "";
        $where = "WHERE product_price_sale != product_price";
    }
