...
class action_menu extends library{
    ...
    /*-----------------------------*/
    public function showMenu_byMultiLevel_mainMenuAChau($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
                ...
                if($level == 0 && $row[$this->nameColTitle_menuLanguage] == "Trang chủ")
                 ...
                 echo '<a href="'.$url.'" class="link_main_menu_'.($level+1).'">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                    if ($url == '/san-pham') {
                        ...
                        $this->showMenu_byMultiLevel_mainMenuAChau($list_menu,$item[$this->nameColID_menu],$lang, $level+1);
                    }
    ...
    public function sanPham () {
        global $conn_vn;
        $sql = "SELECT * FROM product WHERE product_menu = 1";
        $result = mysqli_query($conn_vn, $sql);
        $rows = array();
                                ...
                                <div class="sub-menu-item">
                                    <a href="/'.$item['friendly_url'].'">
                                        <div class="sub-menu-item-img">
                                            <img src="/images/'.$item['product_img'].'" alt="'.$item['product_name'].'" class="img-responsive">
                                        </div>
